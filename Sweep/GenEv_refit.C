...
Int_t fit(double lr, float deltachi)
{
    // double lr = 0.0006;
    // double lr = 0.01;
...
int GenEv_refit(double lr, float deltachi)
{
    clock_t fbegin;
    clock_t fend;
    fbegin = clock();
    ...
    TH1F *hMM3_sm= new TH1F("hMM3_sm","Smeared Missing Mass of 3rd part.",100,920,960);
    ...
    TH2F *hdeltachivslr1 = new TH2F("hdeltachivslr1","hdeltachivslr1",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr2 = new TH2F("hdeltachivslr2","hdeltachivslr2",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr3 = new TH2F("hdeltachivslr3","hdeltachivslr3",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr4 = new TH2F("hdeltachivslr4","hdeltachivslr4",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr5 = new TH2F("hdeltachivslr5","hdeltachivslr5",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr1a = new TH2F("hdeltachivslr1a","hdeltachivslr1a",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr2a = new TH2F("hdeltachivslr2a","hdeltachivslr2a",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr3a = new TH2F("hdeltachivslr3a","hdeltachivslr3a",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr4a = new TH2F("hdeltachivslr4a","hdeltachivslr4a",30,0,0.3,19,1,96);
    TH2F *hdeltachivslr5a = new TH2F("hdeltachivslr5a","hdeltachivslr5a",30,0,0.3,19,1,96);
    TH1F *hTotal = new TH1F("hTotal","hTotal", 5, 1, 5);

    // for(ULong64_t i=0;i<entries;i++){
    for(ULong64_t i=0;i<70000;i++)
    {
        glob_event++;
        Counting(i);
        tree_old->GetEntry(i);
    ...
    hdeltachivslr1a->Fill(lr, deltachi);
    hdeltachivslr2a->Fill(lr, deltachi, MMMean);
    hdeltachivslr3a->Fill(lr, deltachi, MMStdDev);
    hdeltachivslr4a->Fill(lr, deltachi, ProbMean);
    hdeltachivslr5a->Fill(lr, deltachi, ProbStdDev);

    hTotal->Fill(1, MMMean);
    hTotal->Fill(2, MMStdDev);
    hTotal->Fill(3, ProbMean);
    hTotal->Fill(4, ProbStdDev);

    //  double BinContent;
    // BinContent = hTotal->GetBinContent(1);
    // cout<<"BinContent: "<<BinContent<<endl;

    treef->Fill();

    if(abs(MMMean - 938.1) < 0.12 && abs(MMStdDev - 2.536) < 0.3 && abs(ProbMean - 0.1748) < 0.01 && abs(ProbStdDev - 0.2662) < 0.01)
    {
        hdeltachivslr1->Fill(lr, deltachi);
        hdeltachivslr2->Fill(lr, deltachi, MMMean);
        hdeltachivslr3->Fill(lr, deltachi, MMStdDev);
        hdeltachivslr4->Fill(lr, deltachi, ProbMean);
        hdeltachivslr5->Fill(lr, deltachi, ProbStdDev);
    }
    ...
    hProbMean->Write();
    hProbStdDev->Write();
    hdeltachivslr1->Write();
    hdeltachivslr2->Write();
    hdeltachivslr3->Write();
    hdeltachivslr4->Write();
    hdeltachivslr5->Write();
    hdeltachivslr1a->Write();
    hdeltachivslr2a->Write();
    hdeltachivslr3a->Write();
    hdeltachivslr4a->Write();
    hdeltachivslr5a->Write();
    hTotal->Write();
